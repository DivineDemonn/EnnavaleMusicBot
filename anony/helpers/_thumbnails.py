...
class Thumbnail:
    ...
    # pill badge centered under the progress bar.
    ...
    def __init__(self):
        ...
        self.font_title = safe_load_font("your_project/helpers/Raleway-Bold.ttf", 44)
        self.font_artist = safe_load_font("your_project/helpers/Inter-Light.ttf", 24)
        self.font_time = safe_load_font("your_project/helpers/Inter-Light.ttf", 17)
    ...
    async def generate(self, song: Track, progress: float = 0.25, size=(1280, 720)) -> str:
        ...
        try:
            ...
            title_text = trim_text(draw, song.title, self.font_title, bar_w - 110)
            ...
            # Brand pill (bot name)
            pill_text = self.BOT_NAME.upper()
            pill_tw = get_text_width(draw, pill_text, self.font_pill)
            pill_cx = rx + (bar_w // 2)
            time_y = bar_y + 24

            draw_shapes.rounded_rectangle(
                (pill_cx - pill_tw // 2 - 16, time_y - 6,
                 pill_cx + pill_tw // 2 + 16, time_y + 22),
